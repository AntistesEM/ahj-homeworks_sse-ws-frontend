...
sendButton.addEventListener('click', function() {
  ...
  if (message.trim() !== '') { // Если поле не пустое
    const messageObject = JSON.stringify({ userName: message.toString() });
...
messageInput.addEventListener('keyup', function(event) {
  if (event.key === 'Enter' && messageInput.value.trim() !== '') { // Если нажата клавиша Enter и строка не пустая
...
chatInputText.addEventListener('keyup', function(event) {
  if (event.key === 'Enter' && chatInputText.value.trim() !== '') { // Если нажата клавиша Enter и строка не пустая
...
window.addEventListener('beforeunload', () => {
  if (socket.readyState === WebSocket.OPEN) {
    socket.send(JSON.stringify({close: nameUs}));
  }
});
...
function identification () {
  ...
  const myUser = Array.from(listUserNameElements)
    .find((el) => { return el.textContent === nameUs });
  if (myUser) {    
...
function identificationMessages(params) {
  ...
  const myMessages = Array.from(listMessageUser)
    .filter((el) => el.textContent.slice(0, -2) === nameUs);
